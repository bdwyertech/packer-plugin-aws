...
source "appstream-image-builder" "windows" {
  ...
  source_image_name = data.appstream-image.base.name
...
build {
  ...
  provisioner "powershell" {
    inline = [
      "Write-Host 'Base Image: ${data.appstream-image.base.name}'",
      "Write-Host 'Platform: ${data.appstream-image.base.platform}'",
      "Write-Host 'Created: ${data.appstream-image.base.created_time}'"
    ]
  }
}
```
...
```hcl
data "appstream-image" "latest_windows" {
...
source "appstream-image-builder" "windows" {
  name              = "my-custom-image"
  builder_name      = "my-builder"
  source_image_name = data.appstream-image.latest_windows.name
  instance_type     = "stream.standard.medium"
  region            = "us-east-1"
}
...
build {
  ...
  provisioner "powershell" {
    inline = [
      "Write-Host 'Using latest image: ${data.appstream-image.latest_windows.name}'"
    ]
  }
}
```
