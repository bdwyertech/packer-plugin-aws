...
The `subnet` data source is used to fetch information about an AWS VPC subnet. This data source provides a flexible way to query subnets using various criteria like subnet ID, VPC ID, CIDR block, tags, or custom filters. The interface is compatible with Terraform's `aws_subnet` data source.
